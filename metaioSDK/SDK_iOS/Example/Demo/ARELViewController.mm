...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil instructions:(NSString *)arelTutorialConfig
{
    ...
    if (self) {
        arelFile = [[NSString alloc] initWithString:arelTutorialConfig];
    }
    return self;
}
...
- (void) onSDKReady
{
	...
	m_ArelInterpreter->loadARELFile( [arelFile UTF8String] );
}
...
- (void)viewDidLoad {
    ...
    m_pGestureHandlerIOS = [[GestureHandlerIOS alloc] initWithSDK:m_metaioSDK withView:self.arelWebView withGestures:255];
